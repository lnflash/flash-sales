...
export default function LeadStatusCard({ status, leads, config }: LeadStatusCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-light-border dark:border-gray-700">
      <div className={`p-4 border-b border-light-border dark:border-gray-700 ${colors.bg}`}>
        <div className="flex items-center justify-between">
          ...
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colors.badge}`}>{leads.length}</span>
        </div>
      </div>
      ...
      <div className="p-4 max-h-96 overflow-y-auto">
        {leads.length > 0 ? (
              ...
              <div key={submission.id} className="border-b border-light-border dark:border-gray-700 pb-3 last:border-0">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <p className="font-medium text-light-text-primary dark:text-white text-sm">{submission.ownerName}</p>
                    <div className="flex items-center mt-1">
                      <div className="flex items-center">
                        <div className="w-16 bg-light-bg-tertiary dark:bg-gray-700 rounded-full h-1.5">
                          <div
                            className="bg-gradient-to-r from-flash-green to-flash-yellow h-1.5 rounded-full"
                            style={{ width: `${(submission.interestLevel / 5) * 100}%` }}
                          />
                        </div>
                        <span className="ml-2 text-xs text-light-text-secondary dark:text-gray-300">{submission.interestLevel}/5</span>
                      </div>
                      {submission.packageSeen && <span className="ml-3 text-xs text-green-600 dark:text-green-400">📦 Seen</span>}
                    </div>
                    <p className="text-xs text-light-text-tertiary dark:text-gray-400 mt-1">{formatDate(submission.timestamp)}</p>
                  </div>
                  <Link href={`/dashboard/submissions/${submission.id}`} className="ml-2 text-flash-green hover:text-flash-green-light">
          ...
          <p className="text-light-text-tertiary dark:text-gray-400 text-sm">No leads in this status</p>
        )}
      </div>
    </div>
  );
