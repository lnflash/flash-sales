...
import { ChevronDownIcon, ChevronUpIcon, ArrowsUpDownIcon, EyeIcon, PencilSquareIcon, TrashIcon } from "@heroicons/react/24/outline";
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("phoneNumber", {
        header: "Phone",
        cell: (info) => <span className="text-light-text-primary dark:text-gray-300">{info.getValue() || "N/A"}</span>,
      }),
      columnHelper.accessor("packageSeen", {
        header: "Package Seen",
        cell: (info) => <span className={info.getValue() ? "text-flash-green" : "text-light-text-tertiary"}>{info.getValue() ? "Yes" : "No"}</span>,
      }),
          ...
          return (
            <div className="flex items-center">
              <div className="w-1/2 bg-light-bg-tertiary dark:bg-gray-700 rounded-full h-2">
                <div className="bg-gradient-to-r from-flash-green to-flash-yellow h-2 rounded-full" style={{ width: `${(level / 5) * 100}%` }}></div>
              </div>
          ...
          <>
            ...
            {data.map((submission) => {
              ...
              return (
                <div
                  key={submission.id}
                  className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-light-border dark:border-gray-700 overflow-hidden"
                >
                  <MobileCard>
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex-1">
                        <h3 className="font-medium text-light-text-primary dark:text-white text-lg">{submission.ownerName}</h3>
                        <p className="text-sm text-light-text-secondary dark:text-gray-400 mt-1">{formatDate(submission.timestamp)}</p>
                      </div>
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          displayStatus === "converted"
                    ...
                    <div className="space-y-2">
                      ...
                      <MobileCardRow
                        label="Package Seen"
                        value={
                          <span className={submission.packageSeen ? "text-flash-green" : "text-light-text-tertiary dark:text-gray-500"}>
                      ...
                      <MobileCardRow
                        ...
                        value={
                          <div className="flex items-center">
                            ...
                            <span className="text-sm dark:text-white">{submission.interestLevel}/5</span>
                          </div>
                        }
                      />
                    ...
                    {header.isPlaceholder ? null : (
                      ...
                      >
                        ...
                        {{
                          ...
                          (header.column.getCanSort() ? <ArrowsUpDownIcon className="w-4 h-4 ml-1 text-light-text-tertiary dark:text-gray-500" /> : null)}
                      </div>
                    )}
                  </th>
                ))}
