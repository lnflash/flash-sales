...
export default function DynamicIntakeForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentUser?.username) {
        ...
        const supabase = getSupabase();
        const { data: userData } = await supabase.from("users").select("id").eq("username", currentUser.username).single();
      ...
      if (formData.businessName) {
        try {
          if (isEditMode && id) {
            ...
            const { data: existingDeal } = await supabase.from("deals").select("organization_id").eq("id", id).single();
          ...
          if (!organizationId) {
            const { data: existingOrg, error: selectError } = await supabase.from("organizations").select("id").eq("name", formData.businessName).single();
      ...
      if (formData.phoneNumber || formData.email || formData.ownerName) {
        try {
          if (isEditMode && id) {
            ...
            const { data: existingDeal } = await supabase.from("deals").select("primary_contact_id").eq("id", id).single();
          ...
          if (!contactId) {
            ...
            if (formData.email) {
              const { data: existingContact } = await supabase.from("contacts").select("id").eq("email", formData.email).single();
      ...
      if (contactId) {
        const { data: verifyContact } = await supabase.from("contacts").select("phone_primary, email").eq("id", contactId).single();
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-flash-green"
              >
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Monthly Transactions</label>
                <Input name="monthlyTransactions" type="number" value={formData.monthlyTransactions} onChange={handleInputChange} placeholder="500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Average Transaction Size</label>
                <Input name="averageTicketSize" type="number" value={formData.averageTicketSize} onChange={handleInputChange} placeholder="75" />
              </div>
            </div>
            <div className="mt-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Current Pain Points</label>
                ...
                {PAIN_POINTS.map((painPoint) => (
                  <label key={painPoint} className="flex items-center space-x-2 cursor-pointer">
                    <input
                      ...
                      className="rounded border-gray-300 dark:border-gray-600 text-flash-green focus:ring-flash-green dark:bg-gray-800"
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">{painPoint}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="mt-4">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  className="rounded border-gray-300 dark:border-gray-600 text-flash-green focus:ring-flash-green dark:bg-gray-800"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">I have reviewed the Flash payment processing packages</span>
              </label>
            </div>
            <div className="mt-2">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  className="rounded border-gray-300 dark:border-gray-600 text-flash-green focus:ring-flash-green dark:bg-gray-800"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">I want to sign up for Flash payment processing</span>
              </label>
            </div>
          </div>
        );
  ...
  const AdditionalDetailsFields = ({
    ...
    <>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Current Payment Processor</label>
      ...
      <div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="ml-2 font-semibold text-gray-900 dark:text-white">{formData.interestLevel}/5</span>
        </div>
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Decision Makers</label>
        <Input name="decisionMakers" value={formData.decisionMakers} onChange={handleInputChange} placeholder="e.g., Owner, Manager, CFO" />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Specific Needs or Questions</label>
        <textarea
          name="specificNeeds"
        ...
        <div className="max-w-2xl mx-auto">
          <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
            <CardContent className="p-12 text-center">
              ...
              <p className="mt-4 text-gray-600 dark:text-gray-300">Loading submission data...</p>
            </CardContent>
          </Card>
        </div>
      </div>
        ...
        <CardContent className="p-4 sm:p-6 lg:p-8">
          {success ? (
            ...
            <>
              ...
              >
                {error && (
                  <div className="flex items-center gap-2 p-3 text-sm text-red-600 dark:text-red-300 bg-red-50 dark:bg-red-900/20 rounded-lg">
                ...
                <div className="flex justify-between pt-6">
                  <Button type="button" variant="outline" onClick={(e) => handlePrevious(e)} disabled={currentStep === 1}>
                    Previous
                  </Button>
