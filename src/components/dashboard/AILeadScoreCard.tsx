  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="pb-3">
        ...
        {lastUpdated && (
          <CardDescription>
            Last analyzed: {new Date(lastUpdated).toLocaleDateString()}
          </CardDescription>
        )}
      </CardHeader>
      <CardContent>
          ...
          <TabsContent value="prediction" className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                <div className={cn(
                  "text-2xl font-bold",
                  getProbabilityColor(predictedOutcome.probability)
                )}>
                ...
                <div className="w-full bg-gray-700 rounded-full h-2 mt-2">
                  <div 
                    className={cn(
                      ...
                      predictedOutcome.probability >= 0.7 ? 'bg-green-500' :
                      predictedOutcome.probability >= 0.4 ? 'bg-yellow-500' : 'bg-orange-500'
                    )}
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-blue-400">
                  {predictedOutcome.timeToClose} days
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  Average: {historicalComparison.averageTimeToClose} days
                </p>
              </div>
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-green-400">
                  ${predictedOutcome.expectedValue.toLocaleString()}
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  Based on {historicalComparison.similarLeadsCount} similar leads
                </p>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-gray-700">
              ...
              <div className="text-xs space-y-1">
                <p className="text-gray-500">
                  Similar leads convert at {historicalComparison.averageConversionRate}% rate
                </p>
                <p className="text-gray-500">
                  Based on {historicalComparison.similarLeadsCount} comparable opportunities
                </p>
              </div>
            </div>
          </TabsContent>
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-16 bg-gray-700 rounded-full h-2">
                    <div 
                      className={cn(
                        "h-2 rounded-full transition-all",
                        factor.impact >= 0.8 ? 'bg-green-500' :
                        factor.impact >= 0.5 ? 'bg-yellow-500' : 'bg-orange-500'
                      )}
          ...
          <TabsContent value="actions" className="space-y-3">
            ...
            <div className="space-y-2">
              {recommendations.map((recommendation, index) => (
                <div 
                  key={index} 
                  className="p-3 bg-gray-800 rounded-lg text-sm hover:bg-gray-750 transition-colors"
                >
                  {recommendation}
                </div>
              ))}
