...
-- Version 2: Drops existing functions first to avoid conflicts

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS update_deal(UUID, TEXT, UUID, UUID, BOOLEAN, TEXT, INTEGER, TEXT, TEXT, TEXT, TEXT, JSONB);
DROP FUNCTION IF EXISTS update_organization_safe(UUID, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS update_contact_safe(UUID, TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS soft_delete_deal(UUID);

-- If there are other variations, you can also drop them like this:
-- Find all update_deal functions first
DO $$
DECLARE
    func_record RECORD;
BEGIN
    FOR func_record IN 
        ...
        FROM pg_proc 
        WHERE proname = 'update_deal'
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS %I(%s)', func_record.proname, func_record.argtypes);
    END LOOP;
END
$$;
