...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Log the request
    console.log(`Proxying request to: ${TARGET_API_URL}/submissions${req.url?.includes('?') ? req.url.substring(req.url.indexOf('?')) : ''}`);
    ...
    console.error('Error in submissions proxy:', error);
