...
const getFollowUpPriority = (submission: Submission): "urgent" | "high" | "medium" | "low" => {
  const daysSinceContact = Math.floor((new Date().getTime() - new Date(submission.timestamp).getTime()) / (1000 * 60 * 60 * 24));
...
const getTodaysFollowUps = (submissions: Submission[]) => {
  ...
  return submissions
    .map((submission) => {
      ...
      const daysSince = Math.floor((new Date().getTime() - new Date(submission.timestamp).getTime()) / (1000 * 60 * 60 * 24));
...
export default function RepDashboard() {
  ...
  const stats = {
    ...
    conversionRate: submissions.length > 0 ? Math.round((submissions.filter((s) => s.leadStatus === "converted").length / submissions.length) * 100) : 0,
  };
    ...
    return (
      <DashboardLayout title="Sales Rep Dashboard">
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6 text-center">
          <p className="text-yellow-800 dark:text-white">Please log in to view your dashboard.</p>
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout title={`${displayUsername}'s Dashboard`}>
      ...
      {submissions.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm p-12 border border-gray-200 dark:border-gray-700 text-center">
          <div className="max-w-md mx-auto">
            <DocumentTextIcon className="w-16 h-16 text-gray-500 dark:text-gray-400 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No Leads Yet</h3>
            <p className="text-gray-600 dark:text-gray-300 mb-6">Start building your pipeline by adding your first lead!</p>
            <Link
              href="/intake"
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-8">
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 truncate">Total Leads</p>
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-1">{stats.totalLeads}</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 truncate">Hot Leads</p>
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-1">{stats.hotLeads}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">Interest 4-5</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 truncate">Follow-up</p>
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-1">{stats.needsFollowUp}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">Urgent/High</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200 dark:border-gray-700 col-span-2 lg:col-span-1">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-1">{stats.closedThisMonth}</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200 dark:border-gray-700 col-span-2 lg:col-span-1">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-1">{stats.conversionRate}%</p>
                </div>
                <div
                  className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex-shrink-0 ml-2 ${
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {leadStatusOrder
              ...
              .map((status) => {
                ...
                return <LeadStatusCard key={status} status={status} leads={leads} config={config} />;
              })}
          </div>

          {/* Quick Actions */}
          <div className="mt-8 bg-white dark:bg-gray-900 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
